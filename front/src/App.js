...
export const App = () => {
  ...
  const handleSubmit = debounce(() => {
    getWords(dispatch, digits.reduce((accumulator, currentValue) => accumulator + currentValue, ''));
    setDigit([])
  }, 500)
  ...
  return (
    <div className='wrap'>
      ...
      <div className='digits'>
        {digits.map((digit, index) =>       <Chip
          key={index}
          label={digit}
          onDelete={() => setDigit(digits.filter((_, ind) => index !== ind))}
        />)}
      </div>
      <Panel setDigit={setDigit} handleSubmit={handleSubmit} isFetching={app.isFetching}/>
      <Typography component="h1">
        The count of real words is {app.words.length}
      </Typography>
      {app.words.map(word =>          <Chip key={word} label={word}/>)}
    </div>
  );
}
